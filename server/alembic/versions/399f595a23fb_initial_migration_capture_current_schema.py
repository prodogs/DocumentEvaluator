...
Revision ID: 399f595a23fb
Revises: 
Create Date: 2025-05-31 20:56:37.500245
...
def upgrade() -> None:
    ...
    op.alter_column('batches', 'folder_ids',
               existing_type=postgresql.JSONB(astext_type=sa.Text()),
               type_=sa.JSON(),
               existing_nullable=True)
    ...
    op.alter_column('connections', 'connection_config',
               existing_type=postgresql.JSONB(astext_type=sa.Text()),
               type_=sa.Text(),
               existing_nullable=True)
    ...
    op.drop_index(op.f('idx_llm_configurations_active'), table_name='llm_configurations')
    op.drop_index(op.f('idx_llm_configurations_provider_type'), table_name='llm_configurations')
    ...
    op.drop_index(op.f('idx_llm_responses_llm_config_id'), table_name='llm_responses')
    ...
    op.create_foreign_key(None, 'llm_responses', 'llm_configurations', ['llm_name'], ['llm_name'])
...
def downgrade() -> None:
    ...
    op.create_index(op.f('idx_llm_responses_llm_config_id'), 'llm_responses', ['llm_config_id'], unique=False)
    ...
    op.create_index(op.f('idx_llm_configurations_provider_type'), 'llm_configurations', ['provider_type'], unique=False)
    op.create_index(op.f('idx_llm_configurations_active'), 'llm_configurations', ['active'], unique=False)
