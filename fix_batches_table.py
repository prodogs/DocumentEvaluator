#!/usr/bin/env python3
"""
Fix Batches Table - Add Missing meta_data Column

This script adds the missing meta_data column to the batches table
...
def add_meta_data_column(cursor):
    ...
    try:
        # Add the meta_data column as JSON
        cursor.execute(f"""
...
def verify_column(cursor):
    ...
    try:
        ...
        cursor.execute("""
            ...
            FROM information_schema.columns 
            WHERE table_name = 'batches' AND column_name = 'meta_data'
        """)
        
        result = cursor.fetchone()
        if result:
            column_name, data_type, is_nullable, column_default = result
            logger.info(f"Meta_data column verification:")
            ...
            logger.error("Meta_data column verification failed - column not found")
            return False
...
def main():
    ...
    logger.info("🔧 Fixing batches table - adding missing meta_data column")
    ...
    try:
        ...
        success = add_meta_data_column(cursor)
        ...
        if success:
            ...
            if verify_column(cursor):
                ...
                logger.info("The meta_data column has been added to the batches table.")
            else:
                conn.rollback()
